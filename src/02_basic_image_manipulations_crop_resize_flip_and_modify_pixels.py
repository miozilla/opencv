...
print(cb_img[11, 6])  # expect 0
print(cb_img[12, 6])  # expect 255
...
cb_img_copy = cb_img.copy()
cb_img_copy[8, 14] = 200
cb_img_copy[8, 15] = 200
cb_img_copy[9, 14] = 200
cb_img_copy[9, 15] = 200

plt.imshow(cb_img_copy, cmap="Greens")
print(cb_img_copy)

cb_img_copy = cb_img.copy()
cb_img_copy[14, 8] = 200    # [y, x]
cb_img_copy[14, 9] = 200
cb_img_copy[15, 8] = 200
cb_img_copy[15, 9] = 200

plt.imshow(cb_img_copy, cmap="Pastel2")
print(cb_img_copy)
...
cropped_region2 = img_NZ_rgb[240:340, 390:490]      # [y, x]
...
resized_cropped_region_3x = cv2.resize(cropped_region2, None, fx=3, fy=3)
...
# Resize background image to same size as logo image
resized_cropped_region2 = cv2.resize(cropped_region2, dsize=dim, interpolation=cv2.INTER_AREA)
plt.imshow(resized_cropped_region2)
...
# Method 2: Using 'dsize'
desired_width = 150
aspect_ratio = desired_width / cropped_region2.shape[1]
desired_height = int(cropped_region2.shape[0] * aspect_ratio)
dim = (desired_width, desired_height)
...
# Swap channel order
resized_cropped_region_3x = resized_cropped_region_3x[:, :, ::-1]

# Save resized image to disk
cv2.imwrite("resized_cropped_region_3x.png", resized_cropped_region_3x)

# Display the cropped and resized image
Image(filename="resized_cropped_region_3x.png")
...
# Swap channel order
cropped_region2 = cropped_region2[:, :, ::-1]

# Save cropped 'region'
cv2.imwrite("cropped_region2.png", cropped_region2)

# Display the cropped and resized image
Image(filename="cropped_region2.png")
...
# cropped_region2 = cv2.imread("cropped_region2.png")

img_NZ_bgr = cv2.imread("cropped_region2.png", cv2.IMREAD_COLOR)
img_NZ_rgb = img_NZ_bgr[:, :, ::-1]
